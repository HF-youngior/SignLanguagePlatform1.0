...
class MainWindow(QMainWindow):
    ...
    def signalconnect(self):
        ...
        self.ui.FilesBtn_1.clicked.connect(self.detact_batch_imgs)
        self.ui.zhixindudoubleSpinBox.valueChanged.connect(self.zhixinduSpinBoxchange)
        self.ui.zhixinduhorizontalScrollBar.valueChanged.connect(self.zhixinduhorizontalchange)
        self.ui.yanshispinBox.valueChanged.connect(self.yanshispinBoxchange)
        self.ui.yanshihorizontalScrollBar.valueChanged.connect(self.yanshihorizontalchange)

    def yanshispinBoxchange(self):
        self.yanshi = self.ui.yanshispinBox.value()
        self.ui.yanshihorizontalScrollBar.setValue(self.yanshi)

    def yanshihorizontalchange(self):
        self.yanshi = self.ui.yanshihorizontalScrollBar.value()
        self.ui.yanshispinBox.setValue(self.yanshi)

    def zhixinduhorizontalchange(self):
        self.zhixindu = self.ui.zhixinduhorizontalScrollBar.value()/100.0
        self.ui.zhixindudoubleSpinBox.setValue(self.zhixindu)

    def zhixinduSpinBoxchange(self):
        self.zhixindu = self.ui.zhixindudoubleSpinBox.value()
        self.ui.zhixinduhorizontalScrollBar.setValue(int(self.zhixindu*100))
    ...
    def result_guolv(self,result,zhixindu):
        # 假设你的置信度阈值是0.5
        conf_threshold = zhixindu
        # 获取检测结果的boxes对象
        boxes = result.boxes
        # 创建置信度掩码
    ...
    def open_img(self):
        ...
        self.results = self.result_guolv(self.results,self.zhixindu)
    ...
    def open_frame(self):
        ...
        if ret:
            ...
            take_time_str = '{:.3f} s'.format(t2 - t1)
            self.ui.time_lb.setText(take_time_str)
            results = self.result_guolv(results, self.zhixindu)
